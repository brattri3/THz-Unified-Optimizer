...
def main():
    ...
    story.append(Paragraph(
        ...
        "Это однозначно доказывает, что остаточные невязки вызваны дифракцией гауссова пучка на краях металлической оправы поляризатора, которая одинакова для обоих экспериментов.",
        body_style
    ))
