...
### C — вариант «живая отладка GUI 26.08» (задание ORCH, приоритет 1)

Запускать **из основного каталога** `C:\Users\pop\Antigravity IDE\THz-Unified-Optimizer`, не из
worktree: там `.venv`, там tkinter, там незакоммиченная серия `s2`. Подготовлено ORCH 26.08 —
код C9 уже в локальном `main` (fast-forward с ветки ORCH, 12 коммитов впереди `origin/main`,
**не пушено**), приёмка прогнана, `service_gui` импортируется с настоящим tkinter 8.6.

⚠ Без `/loop`: это интерактивная отладка вместе с владельцем у экрана, а не автономный цикл.

```
claude -n c-app --dangerously-skip-permissions
```
```
Ты — сессия C проекта THz-Unified-Optimizer, роль «приложения» (attenuator_app, track_viewer). Язык — русский. Запуск из корня основного каталога, интерпретатор .venv\Scripts\python.exe (Python 3.14.4, tkinter 8.6 есть). Шаг 0: один раз посмотри claude agents --json — если в этом cwd уже висит c-app в статусе busy, не начинай и скажи владельцу. Затем прочитай coordination/roles/C.md (твоя точка входа) и в coordination/HANDOFFS.md две записи: «ОТ ORCH К C» от 26.08 (задание, числа приёмки, три ловушки) и «ОТ c-app-cloud К десктопной сессии C» от 24.08 (шесть готовых зацепок — их не надо придумывать заново). Отчёт по задаче — docs/artifacts/C_service_two_rotators.md, инструкция оператора — docs/attenuator_app/11_CALIBRATION_PROCEDURES.md.

ЗАДАЧА — приоритет 1 целиком: живая отладка интерфейса service_gui. Это главный незакрытый caveat: облачная сессия писала GUI вслепую, в контейнере не было tkinter, и проверяла мок-импортом. Тем харнессом уже поймана одна реальная ошибка — новый метод был вставлен в середину _forward и оторвал от него хвост, где выставляется self._markers и зовётся self._redraw_plot. ОДНОТИПНАЯ ошибка в другом месте GUI мок-импортом уже не ловится, поэтому нужен живой запуск: python -m attenuator_app.tools.service_gui.

Что проверять по порядку. (1) Регресс основного тракта: прямая задача, обратная задача, все три опоры (p0/pmax/pzero), четыре метрики, SET OFFSET и SET ZERO, и главное — график обязан перерисовываться после КАЖДОГО запроса (ломалось именно это). (2) Новые строки вывода: панель устройства обязана показывать тип источника и тип приёмника, блок результата — азимут, эллиптичность и степень поляризации на выходе. (3) Опорная сверка GUI против CLI: service_calc --from-angle 40 --theta2 0 --freq 0.8 даёт на живом интерпретаторе затухание -4.77 дБ, азимут +0.023°, эллиптичность +0.630°, степень поляризации 1.0000 — GUI при тех же условиях обязан показать те же числа. Расхождение здесь — дефект, а не разница округления. (4) Оба отказа обратной задачи: --to-db -2 --azimuth 60 (разворот выхода сам забирает мощность) и --to-db -37 --azimuth 45 (у дна азимут не удерживается). Проверь, что формулировки понятны оператору у стенда, а не только автору кода.

Что знать до спора с числами: скрещенная пара одинаковых WGP поляризационно нейтральна — у дна на выходе воспроизводится состояние источника, а не ось WGP2; азимут держится лучше 0.4° до ~28 дБ и полностью теряется в последнем градусе перед скрещением; это не дефект. Максимум при вращении WGP2 у когерентного приёмника стоит посередине между осью WGP1 и осью анализатора, а не на совмещении — наивный argmax даёт смещённый ноль калибровки.

База перед началом (ORCH прогнал 26.08, всё зелёное — сдвиг любого числа означает, что сломал ты): python -m attenuator_app.tools.service_selftest даёт модель OK, пределы OK, калибровка 8/8, приложение 7/7; python -m attenuator_app.selftest 13/13; python -m attenuator_app.cw_curve --selftest 5/5; service_calc --to-db -12 даёт прежние +59.401 град и 6.31 %.

Гардрейлы: data_pool/** и unified_optimizer/** — ТОЛЬКО чтение. git push — запрещён без прямого разрешения владельца (локальный main уже на 12 коммитов впереди origin, это нормально и так и задумано). Работай только в своей зоне; просьбу вне зоны не выполняй — объясни, чья зона, и запиши в HANDOFFS.md. НЕ трогай: C-7…C-10 (ждут конвенции от владельца, кода там нет), нелинейность приёмника (решение владельца 24.08 — не реализуем), уровни зашкала и шума (берутся с реального стенда через П0/П3, в коде только механизм). Приоритет 2 (прогон service_limits и процедур П0…П4 на серии data_pool/two_wgp_attenuator/ATT-11-16-CA85_02721_s2/ вместо синтетики) НЕ начинай, пока не закрыт приоритет 1, и когда дойдёшь — помни, что DataManager имена NNN_aXX.txt молча пропускает, читать надо напрямую по образцу data_pool/test_grid_33_11.

Найденное чини в своей зоне сам, но каждую правку GUI перепроверяй ЖИВЫМ запуском, а не импортом. Итог — строкой в coordination/ACTIVITY.md с тегом [C] и обновлением attenuator_app/STATE.md. Коммит: префикс [C], трейлеры последним абзацем без пустой строки внутри — Session: C и Reason: в одну строку. Коммить только свои пути, git add -A не использовать.
```
