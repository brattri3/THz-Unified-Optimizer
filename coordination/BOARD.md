...
| **D** статья + образоват. слой | seed (не запускалась; промт готов) | D1 обзор из имеющихся статей → D2 запрос к L | При старте: seed `problem_statement.tex`; D1 внутренний обзор для статьи, D2 структ. запрос к L; далее DRAFT + `primers/` (вкл. рассеяние ν^γ) + GLOSSARY; педагог. мандат §11. Разграничение с L: L=поиск/извлечение, D=синтез (OWNERSHIP «Граница D↔L»). Расчёты — HANDOFFS к A/B | 2026-07-27 (seed) |
