...
def main():
    ...
    # Part 5
    story.append(Paragraph("5. Физическое и метрологическое обсуждение результатов", h1_style))
    story.append(Paragraph(
        "<b>5.1. Интерпретация тестов на нормальность (Почему p ~ 0?)</b><br/>"
        "Крайне малые значения p-value в критериях Шапиро-Уилка и Харке-Бера для обеих серий указывают на отклонение распределения невязок от Гауссова закона. "
        "На Q-Q графиках это выражается в виде S-образных изгибов (тяжелые хвосты). "
        "Это указывает на неслучайный характер ошибок и физическую неполнота модели: в эксперименте присутствуют детерминированные физические процессы, которые не описываются моделью Бланко для бесконечной плоской решетки.",
        body_style
    ))
    story.append(Paragraph(
        "<b>5.2. Пространственное распределение невязок (2D-карты)</b><br/>"
        "1. <i>Зона скрещенных поляризаторов (&theta; ~ 90°):</i> Здесь амплитуда падает почти до уровня шума, а относительные невязки возрастают. Любая паразитная засветка, утечка кросс-поляризации или погрешность юстировки нуля угла приводит к гигантским относительным ошибкам.<br/>"
        "2. <i>Высокочастотный предел (&nu; > 1.5 ТГц):</i> Амплитудный шум возрастает из-за падения спектральной плотности излучения антенн, что размывает фазу и увеличивает среднеквадратичное отклонение.",
        body_style
    ))
    story.append(Paragraph(
        "<b>5.3. Анализ кросс-серийной корреляции невязок</b><br/>"
        "Обнаружена сильная кросс-корреляция фазовых (R_phi ~ 0.594) и амплитудных (R_A ~ 0.282) невязок между независимыми сериями 356att и series3. "
        "Тот факт, что фазовые невязки воспроизводятся почти на 60%, доказывает, что систематическая погрешность заложена в геометрии измерительного стенда или в структуре самого образца поляризатора, а не является случайным шумом.",
        body_style
    ))
    story.append(Paragraph(
        "<b>5.4. Физические гипотезы происхождения систематики</b><br/>"
        "При углах скрещивания (&theta; ~ 50° ... 90°) корреляция невязок резко возрастает до 80–85%. Причины этого:<br/>"
        "1. <i>Непараксиальность пучка и ограничение С/Ш:</i> Реальный пучок ТГц имеет гауссов профиль конечной ширины. Дифракция на оправе исключена (апертура оправы 4 дюйма = 101.6 мм при диаметре пучка 12 мм в центре). Систематика вызвана пределом динамического диапазона и непараксиальностью пучка.<br/>"
        "2. <i>Неоднородность и анизотропия натяжения сетки:</i> Шаг P и диаметр D имеют флуктуации по площади апертуры, а механическое натяжение вольфрамовых нитей может приводить к изгибу (провисанию), что меняет эффективный период.<br/>"
        "3. <i>Паразитное двулучепреломление и рассеяние:</i> На скользящих углах и высоких частотах (&nu; > 1.2 ТГц) длина волны становится сопоставимой с пространственными дефектами решетки, вызывая диффузное рассеяние.",
        body_style
    ))
    
    # Part 6
    story.append(Paragraph("6. Итоговое метрологическое заключение", h1_style))
    story.append(Paragraph(
        "Интеграция модели проводимости Друде и фазовой задержки tau_ps позволила довести точность электродинамического описания решетки до предела (ошибка менее 1 дБ). "
        "Выявленная кросс-серийная корреляция невязок доказывает, что дальнейшее улучшение подгонки невозможно в рамках моделей бесконечных плоских решеток. "
        "Для дальнейшего повышения точности требуется переход к численному моделированию (например, FDTD) с учетом гауссова профиля ТГц-пучка и трехмерной геометрии оправы.",
        body_style
    ))
    ...
    try:
        ...
        pdf_path_fallback = Path("c:/THz-Unified-Optimizer/docs/artifacts/final_comprehensive_report_v2.pdf")
