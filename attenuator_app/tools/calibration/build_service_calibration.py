...
def main() -> int:
    ...
    payload = {
        ...
        "note": (
            ...
            "theta0_calibration_deg -- SET OFFSET: физический офсет совмещения "
            "осей WGP1/WGP2, подгоночный параметр модели, определён по этой "
            "серии; приложение использует его как рабочий офсет по умолчанию. "
            "НЕ путать с SET ZERO -- рабочей точкой отсчёта на угловой кривой, "
            "которую оператор ставит сам и которая в физику не входит. Менять "
            "офсет вручную нужно, только если прибор переустановили в роторе "
            "(алгоритм автокалибровки офсета -- следующая версия). "
            "freqs_ref_thz/power_ref -- средний спектр мощности "
