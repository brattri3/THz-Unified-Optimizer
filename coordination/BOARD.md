...
|---|---|---|---|---|
| **A** модели/идентифицируемость/гипотезы + софт затухания | running (замок перезанят) | **A0_malus_linearization** (приоритетный вопрос владельца, вне очереди) → **A6_angle_symmetry** | A0: гармонический анализ Малюса для неидеального поляризатора (линейный МНК в базисе 2θ/4θ), заметка + `malus_linearization.py`. Затем A6: скрипты закоммичены (d84750f, WIP), нужен прогон → json в results/two_wgp + числа в RESEARCH_LOG + state.json/HYPOTHESES. Далее → A4 (API для C) → A3 → A5 | 2026-07-29 |
