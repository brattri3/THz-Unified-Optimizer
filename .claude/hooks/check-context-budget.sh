...
#
# ⚠ Меряем в LF, а не в том, что лежит на диске (правка 26.08). Файлы
# ужимались в облачном контуре (LF), а на рабочем ПК владельца git
# раскладывает их с CRLF — лишний байт на строку, ~35 на файл роли.
# Из-за этого шесть из семи roles/*.md, ужатые ровно под лимит, локально
# показывали 2418–2437 и все считались нарушителями. Лимит был верен,
# врало измерение; предупреждение из-за этого читалось как шум, а хук
# ровно за тем и заведён, чтобы ему верили. CR отбрасываем перед счётом.
#
