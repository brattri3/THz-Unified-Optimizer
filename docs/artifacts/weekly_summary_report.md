...
> [!IMPORTANT]
> Высокая взаимная корреляция (до 85%) при углах скрещивания 50°–90° **доказывает систематическую природу** остаточных ошибок. Гипотеза о дифракции на оправе является **ложной** (апертура оправы 4 дюйма = 101.6 мм при диаметре пучка 12 мм в центре). Ошибка связана с ограничением динамического диапазона детектора при глубоком затухании сигнала и угловой непараксиальностью пучка.
...
|--------|---------|---------|
| Фазовая корреляция при 50°–90° | R_φ = 0.85 | Предел шума детектора и угловое утолщение (дифракция на оправе ЛОЖНА: апертура 4" >> пучок 12 мм) |
...
- Усложнения **физической** модели (например, 3D RCWA с геометрией проецирования пучка)
...
- Корреляция невязок в 85% при 50°–90° означает, что модель **физически исчерпана** — остаточная ошибка имеет структурную природу (предел динамического диапазона и непараксиальность), которая не описывается формализмом бесконечной плоской решётки.
