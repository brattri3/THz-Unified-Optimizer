...
class Scan(object):
    ...
    def refresh(self, drop_cache=False):
        """Перечитать каталог. `drop_cache=True` — читать все файлы с диска заново.

        Без сброса кэша содержимое берётся по ключу (имя, размер, mtime), и с
        диска читаются только новые и изменившиеся файлы. Физика пересчитывается
        в обоих случаях полностью — разница только в чтении (ТЗ §4.4).
        """
        if drop_cache:
            self._cache.clear()
